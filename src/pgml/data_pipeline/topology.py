...
class TopologyCache:
    ...
    def get_topology(self, topology_id: int) -> HeteroData:
        ...
        node_features = torch.tensor(node_feature_df.to_numpy().copy(), dtype=torch.float32)
        ...
        edge_features = torch.tensor(edge_feature_df.to_numpy().copy(), dtype=torch.float32)
        ...
        # -------------------------
        # Explicit devices & Bipartite attachment
        # -------------------------
        if device_df is not None and device_df.height > 0:
            ...
            device_indices = torch.arange(len(device_node_index), dtype=torch.long)
            bipartite_edge_index = torch.stack([device_indices, device_node_index], dim=0)

            device_type = torch.tensor(device_df["device_type"].cast(pl.Int64).to_list(), dtype=torch.long)
            device_id = torch.tensor(device_df["device_id"].cast(pl.Int64).to_list(), dtype=torch.long)
    ...
    def _load_and_merge_devices(self, topo_dir: Path) -> Optional[pl.DataFrame]:
        ...
        for filename, source_id_col, device_type_value in file_specs:
            ...
            df = self._normalize_device_df(df, source_id_col, device_type_value)
            frames.append(df)
        ...
        for df in frames:
            ...
            if missing_cols:
                fill_exprs = [pl.lit(0).cast(pl.Int64).alias(c) if c in {"id", "node_id", "device_type", "device_id"} else pl.lit(0.0).cast(pl.Float32).alias(c) for c in missing_cols]
                df = df.with_columns(fill_exprs)
            normalized_frames.append(df.select(all_cols))

        merged = pl.concat(normalized_frames, how="vertical")
        cast_exprs = [pl.col(c).cast(pl.Int64) if c in {"id", "node_id", "device_type", "device_id"} else pl.col(c).cast(pl.Float32).fill_null(0.0) for c in merged.columns]
        return merged.with_columns(cast_exprs)

    def _normalize_device_df(self, df: pl.DataFrame, source_id_col: str, device_type_value: int) -> pl.DataFrame:
        if "device_type" not in df.columns:
            if "device_type_idx" in df.columns: df = df.rename({"device_type_idx": "device_type"})
            else: df = df.with_columns(pl.lit(device_type_value).cast(pl.Int64).alias("device_type"))
        return df.rename({source_id_col: "device_id"})