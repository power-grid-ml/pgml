...
from pgml.data_pipeline.step_graph_assembler import StepGraphAssembler
from pgml.data_pipeline.topology import TopologyCache


class StreamingStepDataset(IterableDataset):
    """
    ...
    def __init__(
        ...
        self.assembler = StepGraphAssembler(
    ...
    def __iter__(self) -> Iterator[HeteroData]:
        ...
        for dataset_dir in process_dirs:
            ...
            for step in steps:
                yield self.assembler.assemble_step_graph(dataset_dir=dataset_dir, step=step)