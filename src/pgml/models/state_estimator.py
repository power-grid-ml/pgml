...
class MultiModalStateEstimator(nn.Module):
    ...
    def forward(
        ...
        device_attach_edge = ("device", "attached_to", "node")
        ...
        edge_meas_latent = self.edge_encoder(
            value=batch[edge_type].meas_value, # FIX: use edge_type store
            frequency=batch[edge_type].meas_frequency,
            type_id=batch[edge_type].meas_type,
            mask=batch[edge_type].meas_mask,
        )
        ...
        masked_node_latent, node_obs_indicator = self.node_masker(latent=node_meas_latent, mask_ratio=node_mask_ratio)
        masked_edge_latent, edge_obs_indicator = self.edge_masker(latent=edge_meas_latent, mask_ratio=edge_mask_ratio)
        ...
        node_latent = self.node_device_fusion(
            ...
            device_attached_to_edge_index=batch[device_attach_edge].edge_index,
        )
        ...
        if device_latent.shape[0] > 0 and batch[device_attach_edge].edge_index.shape[1] > 0:
            device_idx = batch[device_attach_edge].edge_index[0]
            node_idx = batch[device_attach_edge].edge_index[1]
            conditioned_device_latent = device_latent.clone()
            conditioned_device_latent[device_idx] += updated_node_latent[node_idx]
        else:
            conditioned_device_latent = device_latent

        pred_node_value = self.node_decoder(
            node_latent=updated_node_latent,
            target_frequency=batch["node"].target_voltage_frequency, # FIX
            target_type=batch["node"].target_voltage_type,
        )

        pred_edge_value = self.edge_decoder(
            edge_latent=edge_latent,
            target_frequency=batch[edge_type].target_current_frequency, # FIX
            target_type=batch[edge_type].target_current_type,
        )

        pred_device_param, pred_device_spec = self.device_decoder(
            device_latent=conditioned_device_latent,
            device_type=batch["device"].device_type,
            param_frequency=batch["device"].target_param_frequency, # FIX
            param_type=batch["device"].target_param_type,
            spec_frequency=batch["device"].target_spec_frequency,
            spec_type=batch["device"].target_spec_type,
        )
