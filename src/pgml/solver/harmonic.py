...
def solve_harmonic(
    ...
    *,
    ...
    if precision != "full":
        ...
        fac = lu_factor_system(y, fixed_rows=fixed_rows, precision=precision)
    ...
    *,
    ...
    if fixed_rows is None:
        if resolved == "sparse":
            return FactoredSystem(
                ...
                n,
                backend="sparse",
                sparse=_SciPySparseLU(y_bus if not mixed else y_bus.to(factor_dtype)),
                y_mat=y_bus,
                **kw,
            )
        if resolved == "block":
            return FactoredSystem(
                ...
                n,
                backend="block",
                block=_BlockLU(y_bus, blocks, factor_dtype=fac_dtype),
                **kw,
            )
        lu, piv = torch.linalg.lu_factor(y_bus if not mixed else y_bus.to(factor_dtype))
        return FactoredSystem("norton", lu, piv, n, y_mat=y_bus, **kw)
    ...
    if resolved == "block":
        ...
        return FactoredSystem(
            ...
            block=_BlockLU.for_free_rows(
                y_bus, blocks, mask, free_rows, factor_dtype=fac_dtype
            ),
            **kw,
        )
    ...
    if resolved == "sparse":
        return FactoredSystem(
            ...
            y_fs,
            backend="sparse",
            sparse=_SciPySparseLU(y_ff if not mixed else y_ff.to(factor_dtype)),
            y_mat=y_ff,
            **kw,
        )
    lu, piv = torch.linalg.lu_factor(y_ff if not mixed else y_ff.to(factor_dtype))
    return FactoredSystem(
        "ideal", lu, piv, n, free_rows, fixed_rows, y_fs, y_mat=y_ff, **kw
    )
...
def back_substitute(fac: FactoredSystem, rhs: Tensor) -> Tensor:
    ...
    if fac.precision == "mixed":
        return _MixedPrecisionSolveFn.apply(fac.y_mat, rhs, fac)
    if fac.backend == "sparse":
