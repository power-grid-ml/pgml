...
def si_field(description: str, *, short: Optional[str] = None, long: Optional[str] = None,
             reference: Optional[str] = None, **kwargs):
...
class ConstantParam(GridModel):
    ...
    value: float = Field(description="Frequency-independent value, SI unit of host field.")
...
class AnalyticParam(GridModel):
    ...
    base_value: float = Field(description="Reference-frequency (f0) value the law scales.")
    params: dict[str, float] = Field(default_factory=dict, description="Extra coefficients (SI).")


class CurveParam(GridModel):
    kind: Literal["curve"] = "curve"
    frequencies_hz: list[float] = si_field(
        "Strictly increasing sample frequencies.", short="Hz", long="hertz", min_length=2
    )
    values: list[float] = Field(description="Sampled values [SI of host], one per frequency.")
...
class EquationParam(GridModel):
    ...
    bindings: dict[str, float] = Field(default_factory=dict, description="Symbol->value (SI).")
...
class HarmonicComponent(GridModel):
    ...
    magnitude_pu: float = si_field(
        ...
        short="pu", long="per unit of fundamental", ge=0.0,
    )
...
class LoadVaryingSpectrum(GridModel):
    ...
    loading_levels_pu: list[float] = si_field(
        ...
        short="pu", long="per unit of rated", min_length=2,
    )
    ...
    def _check(self) -> "LoadVaryingSpectrum":
        if len(self.loading_levels_pu) != len(self.spectra):
            raise ValueError("`loading_levels_pu` and `spectra` must have equal length.")
        if any(b <= a for a, b in zip(self.loading_levels_pu, self.loading_levels_pu[1:])):
...
class TimeVaryingSpectrum(GridModel):
    ...
    profile_ref: Optional[str] = Field(default=None, description="External time/profile id.")


class RandomSpectrum(GridModel):
    kind: Literal["random"] = "random"
    order_magnitude_mean_pu: dict[int, float] = Field(description="Per-order mean magnitude (pu).")
    order_magnitude_std_pu: dict[int, float] = Field(description="Per-order magnitude std (pu).")
...
class DistributionSpectrum(GridModel):
    ...
    distribution_ref: str = Field(description="Id of the fitted spectral distribution (service).")
    params: dict[str, float] = Field(default_factory=dict)


Spectrum = Annotated[
    Union[StaticSpectrum, LoadVaryingSpectrum, TimeVaryingSpectrum, RandomSpectrum,
...
class Provenance(GridModel):
    source_convention: SourceConvention = Field(description="Input convention converted FROM.")
...
class Node(GridModel):
    ...
    zone: NodeZone = Field(default=NodeZone.NONE, description="Land-use character (ML stratifier).")
    geometry: Optional[GeoPoint] = Field(default=None, description="GeoJSON point in grid CRS.")
...
class BranchBase(GridModel):
    ...
    from_phases: tuple[Phase, ...] = Field(description="Phases of `from_node`, in matrix order.")
    to_phases: tuple[Phase, ...] = Field(description="Phases of `to_node`, in matrix order.")
...
class Line(BranchBase):
    ...
    series_resistance_ohm_per_m: Optional[PerPhaseMatrix] = si_field(
        "Reference-frequency series resistance matrix R0.", short="Ohm/m", long="ohm per metre",
        reference="per-phase matrix entry", default=None,
    )
    series_inductance_h_per_m: Optional[PerPhaseMatrix] = si_field(
        "Series inductance matrix L. X(h)=2*pi*h*f0*L.", short="H/m", long="henry per metre",
        default=None,
    )
    shunt_capacitance_f_per_m: Optional[PerPhaseMatrix] = si_field(
        "Shunt capacitance matrix C. B(h)=2*pi*h*f0*C.", short="F/m", long="farad per metre",
        default=None,
    )
    shunt_conductance_s_per_m: Optional[PerPhaseMatrix] = si_field(
        "Shunt conductance matrix G. None = zeros.", short="S/m", long="siemens per metre",
        default=None,
    )
    resistance_frequency: ResistanceFrequencyModel = Field(default_factory=ResistanceFrequencyModel)
    geometry: Optional[GeoLineString] = Field(default=None, description="GeoJSON routing line.")
    ...
    def _check(self) -> "Line":
        ...
        core = (self.series_resistance_ohm_per_m, self.series_inductance_h_per_m,
                self.shunt_capacitance_f_per_m)
        if self.type_ref is None and any(x is None for x in core):
            raise ValueError("Line requires either `type_ref` or explicit series R/L and shunt C.")
        for f in ("series_resistance_ohm_per_m", "series_inductance_h_per_m",
                  "shunt_capacitance_f_per_m", "shunt_conductance_s_per_m"):
            m = getattr(self, f)
...
class Transformer(BranchBase):
    ...
    def _check(self) -> "Transformer":
        core = (self.s_rated_va, self.u_rated_from_v, self.u_rated_to_v, self.from_connection,
                self.to_connection, self.series_resistance_ohm, self.series_inductance_h)
        if self.type_ref is None and any(x is None for x in core):
            raise ValueError("Transformer requires either `type_ref` or explicit ratings, "
...
class ShuntReactor(BranchBase):
    ...
    conductance_s: PerPhaseMatrix = si_field("Shunt conductance matrix G.", short="S",
                                             long="siemens")
    capacitance_f: PerPhaseMatrix = si_field("Shunt capacitance matrix C (B(h)=2*pi*h*f0*C).",
                                             short="F", long="farad")


class GenericBranch(BranchBase):
    component: Literal["generic_branch"] = "generic_branch"
    series_resistance_ohm: PerPhaseMatrix = si_field("Series resistance matrix.", short="Ohm",
                                                     long="ohm")
    series_inductance_h: PerPhaseMatrix = si_field("Series inductance matrix.", short="H",
                                                   long="henry")
    shunt_capacitance_from_f: Optional[PerPhaseMatrix] = si_field("Shunt C at from end.",
                                                                  short="F", long="farad",
                                                                  default=None)
    shunt_capacitance_to_f: Optional[PerPhaseMatrix] = si_field("Shunt C at to end.", short="F",
                                                                long="farad", default=None)
...
class ApplianceBase(GridModel):
    ...
    phases: tuple[Phase, ...] = Field(description="Connected phases at `node`, in vector order.")
    in_service: bool = Field(default=True)
    tags: dict[str, str] = Field(default_factory=dict)
...
class Source(ApplianceBase):
    ...
    resistance_ohm: PerPhaseMatrix = si_field("Per-phase Thevenin resistance matrix.", short="Ohm",
                                              long="ohm")
    inductance_h: PerPhaseMatrix = si_field("Per-phase Thevenin inductance matrix.", short="H",
                                            long="henry")
    spectrum: Optional[Spectrum] = Field(default=None, description="Optional source distortion.")
...
class HarmonicShuntModel(GridModel):
    ...
    series_rl_fraction: float = si_field(
        ...
        short="pu", long="fraction", default=0.5, ge=0.0, le=1.0,
    )
    neglect_shunt: bool = Field(default=False, description="True = pure current source, no shunt.")
    motor_x_harm_pu: Optional[float] = si_field(
        "Motor blocked-rotor / sub-transient reactance for the series branch. None = derive "
        "from P,Q. Typical ~0.20.", short="pu", long="per unit of rated kVA", default=None,
    )
    motor_xr_harm: float = Field(default=6.0, description="X/R ratio of motor_x_harm_pu at f0.")
...
def _check_per_phase_power(obj) -> None:
    n = len(obj.phases)
    for tot, per, label in ((obj.p_nom_w, obj.p_nom_per_phase_w, "p"),
...
class Load(ApplianceBase):
    ...
    zip_coefficients: Optional[ZipCoefficients] = Field(default=None,
                                                        description="Required iff load_model==ZIP.")
    ...
    profile_ref: Optional[str] = Field(default=None, description="External operating-point profile.")
    spectrum: Optional[Spectrum] = Field(default=None,
                                         description="Harmonic current source. None = linear.")
...
class Generator(ApplianceBase):
    ...
    spectrum: Optional[Spectrum] = Field(default=None, description="e.g. inverter spectrum.")
    harmonic_model: HarmonicShuntModel = Field(default_factory=HarmonicShuntModel)
...
class LineType(GridModel):
    ...
    n_phases: int = Field(description="Phase count this type's matrices are defined for.", ge=1)
    series_resistance_ohm_per_m: PerPhaseMatrix = si_field("Series resistance matrix R0.",
                                                          short="Ohm/m", long="ohm per metre")
    series_inductance_h_per_m: PerPhaseMatrix = si_field("Series inductance matrix L.",
                                                        short="H/m", long="henry per metre")
    shunt_capacitance_f_per_m: PerPhaseMatrix = si_field("Shunt capacitance matrix C.",
                                                        short="F/m", long="farad per metre")
    shunt_conductance_s_per_m: Optional[PerPhaseMatrix] = si_field("Shunt conductance matrix G.",
                                                                  short="S/m",
                                                                  long="siemens per metre",
                                                                  default=None)
    resistance_frequency: ResistanceFrequencyModel = Field(default_factory=ResistanceFrequencyModel)
    max_i_a: Optional[float] = si_field("Rated ampacity.", short="A", long="ampere", default=None)


class TransformerType(GridModel):
    """Catalog transformer type: ratings, connections, electrical; no connectivity."""

    id: str = Field(description="Unique catalog id, referenced by Transformer.type_ref.")
    s_rated_va: float = si_field("Rated apparent power.", short="VA", long="volt-ampere", gt=0.0)
    u_rated_from_v: float = si_field("Rated voltage, HV side.", short="V", long="volt", gt=0.0)
    u_rated_to_v: float = si_field("Rated voltage, LV side.", short="V", long="volt", gt=0.0)
    ...
    series_resistance_ohm: float = si_field("Series leakage resistance.", short="Ohm", long="ohm",
                                            reference="referred to HV side")
    series_inductance_h: float = si_field("Series leakage inductance.", short="H", long="henry",
                                          reference="referred to HV side")
    magnetizing_conductance_s: float = si_field("Core-loss conductance.", short="S", long="siemens",
                                                default=0.0)
    magnetizing_inductance_h: Optional[float] = si_field("Magnetizing inductance.", short="H",
                                                         long="henry", default=None)
    zero_sequence: Optional[TransformerZeroSeq] = Field(default=None)
    harmonic_xr_constant: bool = Field(default=False)
    resistance_frequency: ResistanceFrequencyModel = Field(default_factory=ResistanceFrequencyModel)
    nominal_tap: ComplexTap = Field(default_factory=lambda: ComplexTap(ratio_magnitude=1.0),
...
class TransformerShortCircuitInput(GridModel):
    ...
    uk0_percent: Optional[float] = Field(default=None, description="Zero-seq u_k override [%].")
    pk0_w: Optional[float] = Field(default=None, description="Zero-seq copper loss override [W].")
    vector_group: Optional[str] = Field(default=None, description="e.g. 'Dyn11'. Kept in Provenance.")
...
class GridMetadata(GridModel):
    ...
    crs: Optional[str] = Field(default=None, description="Grid-wide CRS id for all geometry.")
    environment: Optional[GridEnvironment] = Field(default=None,
                                                   description="Settlement character (ML feature).")
    tags: dict[str, str] = Field(default_factory=dict)
...
class Grid(GridModel):
    ...
    base_frequency_hz: float = si_field("System fundamental f0.", short="Hz", long="hertz",
                                        default=50.0, gt=0.0)
    nodes: list[Node] = Field(description="All buses.")
    branches: list[Branch] = Field(default_factory=list)
    appliances: list[Appliance] = Field(default_factory=list)
    types: TypeLibrary = Field(default_factory=TypeLibrary, description="Standard-type catalog.")
    ...
    def _integrity(self) -> "Grid":
        ...
        for b in self.branches:
            ...
            if isinstance(b, Line) and b.type_ref is not None and b.type_ref not in self.types.lines:
                raise ValueError(f"Line {b.id} references unknown line type '{b.type_ref}'.")
            if isinstance(b, Transformer) and b.type_ref is not None \
                    and b.type_ref not in self.types.transformers:
                raise ValueError(f"Transformer {b.id} references unknown type '{b.type_ref}'.")
...
__all__ = [
    "Phase", "WindingConnection", "LoadModel", "NodeZone", "GridEnvironment",
    "InterpolationMethod", "ExtrapolationMethod", "SourceConvention",
    "GeoPoint", "GeoLineString",
    "ConstantParam", "AnalyticParam", "CurveParam", "EquationParam", "FrequencyParam",
    "ResistanceFrequencyModel",
    "HarmonicComponent", "SpectrumPoint", "StaticSpectrum", "LoadVaryingSpectrum",
    "TimeVaryingSpectrum", "RandomSpectrum", "DistributionSpectrum", "Spectrum",
    "Provenance", "Node", "BranchBase", "Line", "ComplexTap", "GroundingImpedance",
    "TransformerZeroSeq", "Transformer", "Switch", "ShuntReactor", "GenericBranch", "Branch",
    "ApplianceBase", "Source", "HarmonicShuntModel", "ZipCoefficients", "Load", "Generator",
    "ShuntAppliance", "Appliance",
    "LineType", "TransformerType", "TypeLibrary",
    "LineSequenceInput", "SourceSequenceInput", "SourceShortCircuitInput",
    "TransformerShortCircuitInput", "TransformerImpedanceInput",
    "GridMetadata", "Grid", "si_field",
]
