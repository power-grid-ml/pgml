...
| {mod}`pgml.solver` | `solve_harmonic_flow`, `solve_power_flow`, diagnostics, prepared systems, switch-state batching |
| {mod}`pgml.assembly` | `assemble_ybus`, device injections, branch currents, the node-phase row layout |
