...
model. The converter's documented default (``positive_sequence`` for a line that is not
three-phase, here the four-conductor one) adds the Bessel skin rise and would make the
line itself a second difference between the two engines: measured on this feeder it
moves the comparison from 3.6e-9 V to 3.4e-3 V at order 3 and from 5.3e-9 V to
5.2e-3 V at order 9, which would swamp the zero-sequence quantity under test.
"""
