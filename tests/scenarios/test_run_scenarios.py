...
def _cfg(n=16, seed=7, method="sobol"):
    return ScenarioConfig(
        n_samples=n, seed=seed, method=method,
        parameters=[ParameterSpec(
            name="load_scale", selector=Selector(component="load"),
            distribution=Uniform(low=0.6, high=1.4), field="pq", mode="scale", per="each",
        )],
    )
...
def test_harmonic_shape(grid3):
    res = run_scenarios(
        grid3, _cfg(n=8), calculation="harmonic", harmonic_orders=[1, 5, 7],
        slack="norton", dtype=CDT,
    )
...
def test_cartesian_product_batch(grid3):
    ...
    cfg = CartesianConfig(axes=[
        CartesianAxis(name="l10", selector=Selector(component="load", ids=[10]),
                      values=[0.5, 1.0, 1.5], field="pq", mode="scale"),
        CartesianAxis(name="l11", selector=Selector(component="load", ids=[11]),
                      values=[0.8, 1.2], field="pq", mode="scale"),
    ])
...
def test_differentiable_through_batch(grid3):
    ...
    res = run_scenarios(grid3, _cfg(n=6), calculation="power_flow", slack="ideal", dtype=CDT)
    res.v.abs().sum().backward()
    assert r.grad is not None and torch.isfinite(r.grad).all() and r.grad.abs().sum() > 0
